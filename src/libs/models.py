...
class User_Model(db.Model):
    ...
    re_passwd = db.Column(db.String(1000), nullable=False)
    role = db.Column(db.String(50), nullable=False)

    def json(self):
        return {'id': self.id, 'fname': self.fname, 'lname': self.lname, 'ph_no': self.ph_no, 'email':self.email, 'passwd':self.passwd, 're_passwd':self.re_passwd, 'role':self.role}

    def add_usr(_fname, _lname, _ph_no, _email, _passwd, _re_passwd, _role):
        '''method to add User to database using parameters'''
        # creating an instance of our User constructor
        new_user = User_Model(fname= _fname, lname =_lname, ph_no =_ph_no, email=_email, passwd= _passwd, re_passwd=_re_passwd, role=_role)
